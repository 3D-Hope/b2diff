...
# run the following to store the intermediate resampled images for visualization:
# """
# Feynman-Kac Diffusion (FKD) steering mechanism implementation.
# """
...
#         # Save images to img_dir for reward computation
#         debug_img_dir = os.path.join(img_dir, f"sampling_idx_{sampling_idx:03d}")
#         os.makedirs(debug_img_dir, exist_ok=True)
#         os.makedirs(img_dir, exist_ok=True)
#         for idx, img in enumerate(population_images):
#             img_array = (img.cpu().numpy().transpose(1, 2, 0) * 255).astype('uint8')
#             pil_img = PILImage.fromarray(img_array)
#             pil_img.save(os.path.join(debug_img_dir, f"{idx:05d}.png"))
        
#         eval_scores, score_metrics, raw_clip_scores = self.reward_fn(ground, debug_img_dir, save_dir, config) # TODO: it could be more efficient
        
#         # Clean up temporary images #TODO: use clean up
#         # for filename in os.listdir(img_dir):
#         #     file_path = os.path.join(img_dir, filename)
#         #     if os.path.isfile(file_path):
#         #         os.remove(file_path)
...
#         rs_candidates = raw_clip_scores
#         print(f"Sampling idx {sampling_idx}: Reward scores: {raw_clip_scores}")
#         if not get_best_indices:
#             rs_candidates = torch.tensor([-score for score in raw_clip_scores], device=self.device)
#         else:
#             # Ensure rs_candidates is on the correct device
...
#                 self.product_of_potentials[indices] * w[indices]
#             )
#         print(f"Resampling indices at timestep {sampling_idx}: {indices}")
#         return resampled_latents, resampled_images, indices
