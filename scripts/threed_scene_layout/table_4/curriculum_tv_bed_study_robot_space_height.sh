run_name="curriculum_tv_bed_study_robot_space_height"
python3 ./scripts/training/train_pipeline.py \
    exp_name=${run_name} \
    seed=42 \
    sample.batch_size=32 \
    train.batch_size=32 \
    sample.num_batches_per_epoch=16 \
    wandb.enabled=true \
    threed_scene_layout=true \
    sample.no_branching=true \
    sample.no_selection=true \
    split_time=1 \
    sample.normalize_all=true \
    train.incremental_training=true \
    sample.num_steps=20 \
    train.num_stages_per_increment=20 \
    universal_rewards=true \
    'custom_reward=[tv_bed,desk_chair_for_study,robot_reachability_075m,robot_fetch_from_table_1m_high]' \
    pipeline.stage_cnt=200 \
    continue_from_universal=true \
    path_to_universal_lora=#TODO: GET THE LORA PATH AFTER CHOOSING THE OPERATING POINT ON TV BED + study + robot space