...
if [[ -n "${STAGES_OVERRIDE:-}" ]]; then
    ...
    all_stages=$(ls "${model_root}" | sed -n 's/^stage\([0-9]\+\)$/\1/p' | sort -n)
    stages=""
    for stage_number in ${all_stages}; do
        if (( stage_number % stage_stride == 0 )); then
            stages="${stages} ${stage_number}"
        fi
    done
    stages="$(echo "${stages}" | xargs)"
    echo "Auto-selected every ${stage_stride} stages: ${stages}"
fi

if [[ -z "${stages}" ]]; then
    echo "No matching stages found in ${model_root}"
    exit 1
fi
...
echo "Every-${stage_stride} stage inference completed for ${run_name}"
