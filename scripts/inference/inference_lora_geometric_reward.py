...
def generate_and_evaluate_lora(
    ...
    print("="*80)
    print("LORA INFERENCE + GEOMETRIC REWARD COMPUTATION")
    print("="*80)
    ...
    # ============================================================================
    # STEP 3: Compute Geometric Rewards
    # ============================================================================
    print("\n[STEP 3] Compute Geometric Rewards")
    
    # Load Geometric reward calculator
    reward_calculator = ImageGeometricReward(min_length=20.0)
    print("✓ Geometric reward calculator loaded")
    
    # Compute rewards
    similarity_scores = []
    
    for img_tensor in tqdm(all_images, desc="Evaluating"):
        # Convert tensor to numpy RGB array for the reward calculator
        image_array = (img_tensor.cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
        
        try:
            reward = reward_calculator.get_algebraic_intersection_reward(
                image_array, num_samples=5, threshold_c=0.03
            )
        except Exception:
            reward = 0.0
            
        similarity_scores.append(float(reward))
    ...
    print(f"✓ Geometric rewards computed")
    ...
    print(f"Geometric Reward Mean: {mean_reward:.4f}")
    print(f"Geometric Reward Std:  {std_reward:.4f}")
    print(f"Geometric Reward Min:  {similarity_scores.min():.4f}")
    print(f"Geometric Reward Max:  {similarity_scores.max():.4f}")
    print("="*80)
    
    # Save results
    results_file = os.path.join(output_dir, "geometric_rewards.json")
    results = {
        "checkpoint": checkpoint_path,
        "num_images": len(all_images),
        "geometric_reward_mean": mean_reward,
        "geometric_reward_std": std_reward,
        "geometric_reward_min": float(similarity_scores.min()),
        "geometric_reward_max": float(similarity_scores.max()),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate images from LoRA and compute Geometric rewards")
    parser.add_argument(
        "--checkpoint_path",
        type=str,
